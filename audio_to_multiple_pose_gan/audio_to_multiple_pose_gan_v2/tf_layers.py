...
def keypoints_to_train(poses, arr):
    ...
    reshaped = tf.reshape(poses, (tf.shape(input=poses)[0], shape[1].value, 2, 49))
    required_keypoints = tf.gather(reshaped, indices=arr, axis=3)
    return tf.reshape(required_keypoints, (tf.shape(input=poses)[0], shape[1].value, 2*len(arr)))
...
def keypoints_regloss(gt_keypoints, pred_keypoints, regloss_type):
    ...
    return tf.reduce_mean(input_tensor=loss_func(tf.compat.v1.layers.flatten(gt_keypoints), tf.compat.v1.layers.flatten(pred_keypoints)))
...
def GroupNorm(x, type='1d', G=32, esp=1e-5):
    ...
    if type == '1d':
        ...
        mean, var = tf.nn.moments(x=x, axes=[2, 3], keepdims=True)
...
def Norm(input, norm='batch', type='1d', is_training=True):
    if norm == 'batch':
        output = tf.keras.layers.BatchNormalization(input, training=is_training)
    elif norm == 'instance':
        output = tfa.layers.InstanceNormalization(input, trainable=False)
    elif norm == 'group':
        if type == '1d':
...
def ConvNormRelu(input, channels, type='1d', is_training=True, leaky=False, downsample=False, norm='batch', k=None,
    ...
    if type == '1d':
        conv = tf.keras.layers.Conv1D(input, filters=channels, kernel_size=k, strides=s, padding=padding,
                                kernel_initializer=tf.keras.initializers.GlorotUniform(),
                                bias_initializer=tf.compat.v1.zeros_initializer(), activation=None)
    elif type == '2d':
        conv = tf.keras.layers.Conv2D(input, filters=channels, kernel_size=k, strides=s, padding=padding,
                                kernel_initializer=tf.keras.initializers.GlorotUniform(),
                                bias_initializer=tf.compat.v1.zeros_initializer(), activation=None)
    else:
