...
class PoseGAN():
    ...
    def __init__(self, args, seq_len=64):
        ...
        if self.args.reg_loss in ['pose', 'both']:
            ...
            tf.compat.v1.summary.scalar(name='pose_reg', tensor=pose_reg, collections=['g_summaries'])
        ...
        if self.args.reg_loss in ['motion', 'both']:
            ...
            tf.compat.v1.summary.scalar(name='motion_reg', tensor=motion_reg, collections=['g_summaries'])
            self.pose_regloss += motion_reg

        # Global Discriminator and Hand Discriminator
        self.G_gan_loss = tf.convert_to_tensor(value=0.)
        if self.args.gans:
    ...
    def train(self, epochs=1000):
        ...
        g_summaries = tf.compat.v1.summary.merge(tf.compat.v1.get_collection("g_summaries"))
        if self.args.gans:
            d_summaries = tf.compat.v1.summary.merge(tf.compat.v1.get_collection("d_summaries"))
    ...
    def save_prediction_video(self, df, keypoints_pred, keypoints_gt, save_path, limit=None, loss=None):
        if limit == None:
            limit = len(df)
        print("want to save video at ", save_path)
        for i in range(min(len(df), limit)):
            try:
                ...
                if not (os.path.exists(dir_name)):
                    print("made video output path: ", dir_name)
                ...
                print("saving side by side video at ", temp_otpt_fn)
    ...
    def save_prediction_video_by_percentiles(self, df, keypoints_pred, keypoints_gt, save_path, loss_percentile_bgt=95,
        ...
        # CARO TODO: this is currently creating the frames of a video in the file but not actually converting
        #  it to a video and then overwriting those frames with the next video's frames.

        print("in save prediction by percentile")
        ...
        print("saving prediction video at ", save_path)
    ...
    def predict_df(self, df, cfg, shift_pred=(900, 290), shift_gt=(1900, 280)):
        ...
        print("returning keypoints list")
