...
class PoseGAN():
    ...
    def __init__(self, args, seq_len=64):
        ...
        if self.args.reg_loss in ['pose', 'both']:
            ...
            tf.summary.scalar(name='pose_reg', data=pose_reg)  # TF1, collections=['g_summaries'])
        ...
        if self.args.reg_loss in ['motion', 'both']:
            ...
            tf.summary.scalar(name='motion_reg', data=motion_reg)  # TF 1, collections=['g_summaries'])
        ...
        if self.args.gans:
            ...
            self.D_loss = tf.compat.v1.losses.mean_squared_error(tf.ones_like(self.real_pose_score), self.real_pose_score) \
                          + args.lambda_d * tf.compat.v1.losses.mean_squared_error(tf.zeros_like(self.fake_pose_score),
                                                                         self.fake_pose_score)

            tf.compat.v1.summary.scalar(name='D_loss', tensor=self.D_loss, collections=['d_summaries'])

            # loss for training the generator from the global D - have I fooled the global D?
            self.G_gan_loss = tf.compat.v1.losses.mean_squared_error(tf.ones_like(self.fake_pose_score), self.fake_pose_score)
            tf.compat.v1.summary.scalar(name='G_gan_loss', tensor=self.G_gan_loss, collections=['g_summaries'])

            # train global D
            with tf.control_dependencies(tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.UPDATE_OPS, scope=D_SCOPE)):
                self.train_D = tf.compat.v1.train.AdamOptimizer(learning_rate=self.args.lr_d)\
                    .minimize(loss=self.D_loss, var_list=tf.compat.v1.trainable_variables(scope=D_SCOPE))


        # sum up ALL the losses for training the generator
        self.G_loss = self.pose_regloss + args.lambda_gan * self.G_gan_loss
        tf.compat.v1.summary.scalar(name='train_loss', tensor=self.G_loss, collections=['g_summaries'])

        # train the generator
        if self.args.mode != 'inference':
            trainable_variables = tf.compat.v1.trainable_variables(scope=G_SCOPE)
            with tf.control_dependencies(tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.UPDATE_OPS, scope=G_SCOPE)):
                self.train_G = tf.compat.v1.train.AdamOptimizer(learning_rate=self.args.lr_g).minimize(loss=self.G_loss,
    ...
    def train(self, epochs=1000):
        ...
        if hasattr(self.args, 'checkpoint') and self.args.checkpoint:
            ...
            self.restore(self.args.checkpoint, scope_list=str(scope_list))

        g_summaries = tf.compat.v1.summary.merge(inputs=tf.compat.v1.get_collection("g_summaries"), collections=['g_summaries'])
        if self.args.gans:
            d_summaries = tf.compat.v1.summary.merge(tf.compat.v1.get_collection("d_summaries"), collections=['d_summaries'])
            print("")
            print("got d_summaries from collection: ")
            print(d_summaries)
            print("")

        writer = tf.compat.v1.summary.FileWriter(base_path, self.sess.graph)
        ITR_PER_EPOCH = 300
        ...
        for j in range(epochs):
            ...
            summary = tf.compat.v1.Summary(value=[
                tf.compat.v1.Summary.Value(tag="validation_loss", simple_value=avg_loss),
            ])
            writer.add_summary(summary, global_step=ITR_PER_EPOCH * j)
            summary = tf.compat.v1.Summary(value=[
                tf.compat.v1.Summary.Value(tag="pck_loss", simple_value=pck_loss),
            ])
            writer.add_summary(summary, global_step=ITR_PER_EPOCH * j)
            ...
            if lowest_validation_loss > avg_loss:
                ...
                tf.compat.v1.train.Saver().save(self.sess, os.path.join(base_path,
                                                              'best_ckpt-step_%s_validation_loss_%.3f.ckp' % (
                                                              ITR_PER_EPOCH * j, avg_loss)))

            if j % 15 == 14:
                tf.compat.v1.train.Saver().save(self.sess, os.path.join(base_path, 'ckpt-step-%s.ckp' % (ITR_PER_EPOCH * j)))
            ...
            for i in tqdm(range(ITR_PER_EPOCH)):
                if self.args.gans:
                    for itr_d in range(self.args.itr_d):
                        audio_X, pose_Y = next(train_generator)
                        d_loss, d_summaries_str, _ = self.sess.run([self.D_loss,
                                                                    d_summaries,
                                                                    self.train_D],
    ...
    def restore(self, ckp, scope_list=('generator')):
        variables = []
        for s in scope_list:
            variables += tf.compat.v1.global_variables(scope=s)

        #  CARO: INSPECT THIS CHECKPOINT
        # print("VARIABLES IN SCOPE LIST: ", variables)
        print("******************************************************************************")
        print("ckpt file: ", ckp)

        print("VARIABLES IN CHECKPOINT FILE:")
        latest_ckp = tf.train.latest_checkpoint(ckp)
        print_tensors_in_checkpoint_file(latest_ckp, all_tensors=True, tensor_name='')

        print("******************************************************************************")

        tf.compat.v1.train.Saver(variables).restore(self.sess, ckp)

    def save_prediction_video(self, df, keypoints_pred, keypoints_gt, save_path, limit=None, loss=None):

        print("in save prediction video in same dir")

        if limit == None:
            limit = len(df)
        for i in range(min(int(len(df)), int(limit))):
            try:
                row = df.iloc[i]
                ...
                # CARO TODO make the last %s back into {loss}
                otpt_fn = os.path.join(save_path,
                                       '%s_%s_%s_%s_%s.mp4' % (video_fn, interval_id, row['start'], row['end'], str(i)))
                save_side_by_side_video(dir_name, keypoints1, keypoints2, temp_otpt_fn, delete_tmp=False)
                audio = np.load(row['pose_fn'])['audio']
                print("audio naming things: ")
                print("interval id: ", row["interval_id"])
                print("start: ", row["start"])
                print("end: ", row["end"])

                audio_out_path = os.path.join('tmp', 'audio_cache',' %s_%s_%s.wav' % (row["interval_id"], row['start'], row['end']))
                audio_out_path = audio_out_path.replace(":", "_")
                save_audio_sample(audio, audio_out_path, 16000, 44100)

                # CARO TODO change this back to loss once I understand it
                # if loss is not None:
                #     loss_sub = str(loss[i]).replace(" ", "-").replace("[", "").replace("]", "")
                #     otpt_fn = otpt_fn.format(loss=loss_sub)

                print("saving video from audio: ", audio_out_path)
                print("video: ", temp_otpt_fn)
                otpt_fn = otpt_fn.replace(":", "_")
                print("to: ", otpt_fn)
                save_video_from_audio_video(audio_out_path, temp_otpt_fn, otpt_fn)
                # subprocess.call('rm -R "%s"' % (dir_name), shell=True)
                print("previously tried to remove ", dir_name)
