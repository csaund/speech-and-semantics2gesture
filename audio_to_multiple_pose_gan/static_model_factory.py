...
def D_patchgan(x_pose, n_downsampling=2, norm='batch', reuse=False, is_training=False, scope='discriminator'):
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        ndf = 64
        model = tf.keras.layers.Conv1D(filters=ndf, kernel_size=4, strides=2, padding='same',
        ...
        conv = tf.keras.layers.Conv1D(filters=1, kernel_size=4, strides=1,
...
def audio_to_pose_gans(input_dict, reuse=False, is_training=False):
    with tf.compat.v1.variable_scope('generator', reuse=reuse):
        # print("dict type: ", type(input_dict))
        ...
        with tf.compat.v1.variable_scope('logits'):
            conv = tf.keras.layers.Conv1D(filters=98, kernel_size=1, strides=1,
                    kernel_initializer=tf.keras.initializers.GlorotUniform(), bias_initializer=tf.zeros_initializer(),
