...
def keypoints_regloss(gt_keypoints, pred_keypoints, regloss_type):
    ...
    return tf.reduce_mean(loss_func(tf.keras.layers.Flatten()(gt_keypoints), tf.keras.layers.Flatten()(pred_keypoints)))
...
def Norm(input, norm='batch', type='1d', is_training=True):
    if norm == 'batch':
        ...
        output = output(inputs=input, training=is_training)
    elif norm == 'instance':
        output = tfa.layers.InstanceNormalization(trainable=False)
...
def ConvNormRelu(input, channels, type='1d', is_training=True, leaky=False, downsample=False, norm='batch', k=None,
    ...
    if type == '1d':
        conv = tf.keras.layers.Conv1D(filters=channels, kernel_size=k, strides=s, padding=padding,
                                kernel_initializer=tf.keras.initializers.GlorotUniform(),
                                bias_initializer=tf.zeros_initializer(), activation=None)
        conv = conv(input)
    elif type == '2d':
        conv = tf.keras.layers.Conv2D(filters=channels, kernel_size=k, strides=s, padding=padding,
                                kernel_initializer=tf.keras.initializers.GlorotUniform(),
                                bias_initializer=tf.zeros_initializer(), activation=None)
        conv = conv(input)
    else:
